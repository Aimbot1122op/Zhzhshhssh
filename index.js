...
const token = '7007471636:AAFlpb6Z_j4x-QCaRbG-7-934DP3vRdCYj8'; // Replace with your Telegram bot token
const bot = new TelegramBot(token, { polling: true });
const channelUsername = '@jujutsukaisenaimbotd'; // Replace with your channel username
...
bot.onText(/\/start/, (msg) => {
  ...
  bot.sendMessage(msg.chat.id, 'Welcome to the MediaFire Link Bot! Please join our channel @jujutsukaisenaimbotd to use this bot.', opts);
});
...
bot.on('callback_query', async (callbackQuery) => {
  ...
  if (callbackQuery.data === 'check_membership') {
    ...
    if (isMember) {
      ...
      bot.sendMessage(message.chat.id, 'You are not a member. Please join @jujutsukaisenaimbotd to use this bot.');
    }
  }
});
...
bot.onText(/https?:\/\/(www\.)?mediafire\.com\/file\/[^\s]+/, async (msg, match) => {
  ...
  if (!isMember) {
    ...
    bot.sendMessage(chatId, 'You are not a member. Please join @jujutsukaisenaimbotd to use this bot.', opts);
    return;
  }
